...
export default function CashFlowPage() {
  ...
  return <div className="mx-auto w-full max-w-[1600px] space-y-6 p-4 sm:p-8"><PageHeader title="Fluxo de caixa" subtitle="Recebimentos, despesas e valores em aberto." actions={
    ...
    <div id="cash-flow-results" className="scroll-mt-4"><Card className="relative p-5"><h2 className="text-base font-semibold">Movimentações · {query.data?.total??0}</h2>{query.isLoading?<div className="min-h-40"/>:<div className={`transition-opacity ${query.isFetching?'opacity-45 pointer-events-none':''}`} aria-busy={query.isFetching}><FinanceHistory entries={query.data?.data??[]} from={from} to={to}/></div>}<Pagination current={page} total={Math.max(1,Math.ceil((query.data?.total??0)/25))} onPageChange={setPage} loading={query.isFetching} scrollTargetId="cash-flow-results"/></Card></div>
